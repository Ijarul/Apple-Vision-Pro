...
// CHANGING THE OPACITY OF THE vision pro
gsap.to("#page-buttom",{
  scrollTrigger:{
    trigger:`#page-buttom`,
    start:`5% top`,
...
// time to move the text in the dispaly
t1.to("#page1>h1",{
  top:`-50%`
})
...
//canvas part
function canvas(){
  const canvas = document.querySelector("#page7>canvas"); //access the canvas given in page7
const context = canvas.getContext("2d");

canvas.width = window.innerWidth;//give height and width
canvas.height = window.innerHeight;


window.addEventListener("resize", function () {//adjusting the height and width or customize
canvas.width = window.innerWidth;
canvas.height = window.innerHeight;
render();
});

function files(index) { //storing all the photo
var data = `
...
`;
return data.split("\n")[index]; //coveting text into image
}

const frameCount = 198;  //frame count
